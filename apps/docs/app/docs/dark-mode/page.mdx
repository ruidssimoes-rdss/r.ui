...
r/ui fully supports dark mode with automatic system preference detection, manual toggling, and persistence.
...
When set to `"system"`:
- On iOS: Follows Settings > Display & Brightness
- On Android: Follows Settings > Display > Dark theme
...
A simple toggle button that switches between dark and light:

```tsx
import { Button, useColorScheme, View } from '@r-ui/react-native'
